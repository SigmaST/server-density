...
MCON="yum -y install https://raw.githubusercontent.com/jelastic-jps/server-density/master/dumps/mysql-community-libs-compat-5.7.7-0.3.rc.el7.x86_64.rpm MySQL-python.x86_64"
MYSQL_PLUGIN_SCRIPT="curl -fsS 'https://raw.githubusercontent.com/jelastic-jps/server-density/master/scripts/MySQL.py' -o ${KEY_DIRECTORY}/MySQL.py"
MONGO_PLUGIN_SCRIPT="pip install pymongo && curl -fsS 'https://raw.githubusercontent.com/jelastic-jps/server-density/master/scripts/Mongodb.py' -o ${KEY_DIRECTORY}/Mongodb.py"
...
if [ "${TYPE}" = "memcached" ]; then
     sh -c "ln -s ${SD_CONFIG_FILE} ${KEY_DIRECTORY} && ln -s ${SD_LOG_DIR} ${KEY_DIRECTORY} && chown 777 ${KEY_DIRECTORY} && cd /var/lib/jelastic/keys &&  curl -fsS 'https://raw.githubusercontent.com/jelastic-jps/server-density/master/scripts/Memcached.py' -o Memcached.py"
fi
